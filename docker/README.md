# Xiaomi Location Bridge - 后端服务

基于 Playwright + FastAPI 的后端定位服务，负责与小米云空间（i.mi.com）通信，提供设备定位 API。
...
```bash
cd docker
docker build -t xiaomi-location-bridge .

docker run -d \
  --name xiaomi-location-bridge \
  -p 8000:8080 \
  --restart unless-stopped \
  xiaomi-location-bridge
```
...
services:
  xiaomi-location-bridge:
    build: ./docker
    container_name: xiaomi-location-bridge
    ports:
      - "8000:8080"
